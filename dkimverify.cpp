...
int CDKIMVerify::ParseDKIMSignature( const string& sHeader, SignatureInfo &sig )
{
	...
	if (values[9] != NULL)
	{
		...
		bool HasDNS = false;
		char* s = strtok_s(values[9], ":", &saveptr);
		while (s != NULL)
		{
	...
	char* s = strtok_s(values[4], ":", &saveptr);
	while (s != NULL)
	{
...
int SelectorInfo::Parse( char* Buffer )
{
	...
	else
	{
		...
		char* s = strtok_s(values[2], ":", &saveptr);
		while (s != NULL)
		{
			if (strcmp(s, "sha1") == 0)
				AllowSHA1 = true;
	...
	if (values[5] != NULL)
	{
		...
		char* s = strtok_s(values[5], ":", &saveptr);
		while (s != NULL)
		{
	...
	if (values[6] != NULL)
	{
		char* s = strtok_s(values[6], ":", &saveptr);
		while (s != NULL)
		{
			if (strcmp(s, "y") == 0)
			{
